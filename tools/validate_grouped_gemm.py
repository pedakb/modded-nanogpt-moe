...
execution, compared against the active per-expert Python-loop MoE class
(modded_nanogpt_moe.model.MoE, imported without reimplementation). It does
not change the already-integrated training path.
...
class GroupedGemmMoE(nn.Module):
    ...
    verbatim from the active MoE forward, so output semantics match
    exactly -- only expert execution differs. Ordinary per-expert
    nn.Parameters (one fc/proj weight+bias per expert, same shapes as
    the active MLP's Linear submodules) are stored in ParameterLists;
    ...
    def load_from_moe(self, moe: "tgs.MoE"):
        """Copy weights from an existing active MoE instance so
...
def report_production_dtype_conversions(moe, x):
    ...
    print("Production dtype/layout conversion probe (same expressions as the active MoE):",
          flush=True)
